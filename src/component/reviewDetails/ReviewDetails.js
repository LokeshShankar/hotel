...
import "../../css/reviewDetails/Loader.css";
...
function ReviewDetails(props) {
  ...
  const url = REVIEW_SERVICE_BASE_URL + "/hotel/";
  ...
  if (flag == 1) {
    return <Form flagchange={flagchange} hotelid={props.hotelid}></Form>;
  } else
    return (
      <div id="reviewDetails">
        <div className="review-container">
          <div>
            <h4 className="review-head">Hotel Reviews and Ratings</h4>
            <button
              type="button"
              onClick={(event) => {
                let jwtTokenCookie = new Cookies();

                let jwt = jwtTokenCookie.get("jwtToken");
                if (typeof jwt == "undefined") {
                  alert("You need to login to add review!!");
                } else setFlag(1);
              }}
